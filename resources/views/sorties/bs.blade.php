            ...
            <div class="container-xl">
                <div class="row g-2 align-items-center">
                    ...
                    <div class="col-auto ms-auto d-print-none d-flex">
                        ...
                        <form action="{{ url('sorties/' . $bs->sortie->id) }}" method="post" id="delete-form">
                            @csrf
                            @method('DELETE')
                    ...
                    @can('modifier_sorties')
                        <a href="{{ url('sorties/' . $bs->sortie->id . '/edit', []) }}" class="btn btn-outline-warning me-1">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-sm-6 text-sm-end text-center mb-3">
                                <p class="fs-3"><strong>Bon de sorties : </strong>{{ $bs->sortie->num }}</p>
                                <p class="fs-3"><strong>Tizi-ouzou Le :
                                    </strong>{{ date('d-m-Y', strtotime($bs->sortie->date)) }}</p>
                            ...
                            <table class="table table-transparent table-responsive table-bordered fs-5 ">
                                ...
                                <tbody>
                                    @php
                                        ...
                                        $break = false;
                                    @endphp
                                    @foreach ($bs->sortie->articles as $article)
                                        <tr>
                        ...
                        <div class="row d-flex justify-content-between text-uppercase mt-5">
                            <div class="col-sm-6 text-sm-start text-center  fs-3">
                                <div class="h3 mb-3">
                                    <p class="h3"><strong>Le Magasinier</strong></p>
                                    ...
                                    @if (auth()->user()->signature)
                                        @if ($bs->sortie->signer)
                                            ...
                                            <div class="d-print-none ">
                                                <form action="{{ route('signer', ['id' => $bs->sortie->id]) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('delete')
                            ...
                            <div class="col-sm-6 text-sm-start text-center ps-sm-5 fs-3">
                                <p class="h3 mb-3"><strong>le beneficiaire</strong></p>
                                <p class="ms-sm-3">
                                    <strong>Nom et prenom : </strong>{{ $bs->sortie->nom }} {{ $bs->sortie->prenom }} <br>
                                    <strong>Service : </strong>{{ $bs->service }}
                                </p>
                            </div>
                        </div>
                    </div>
